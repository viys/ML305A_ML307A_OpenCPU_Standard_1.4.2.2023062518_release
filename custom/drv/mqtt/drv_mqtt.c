...
static void __demo_mqtt_event_handler(void* handle,
...
static void __demo_mqtt_default_recv_handler(void* handle,
...
void* my_aliyun_mqtt_connect(char* host, char* productKey, char* deviceName,
                             char* deviceSecret) {
    void* handle = NULL;
    int32_t res = STATE_SUCCESS;
    uint16_t port =
    ...
    /* 配置MQTT默认消息接收回调函数 */
    aiot_mqtt_setopt(handle, AIOT_MQTTOPT_RECV_HANDLER,
                     (void*)__demo_mqtt_default_recv_handler);
    /* 配置MQTT事件回调函数 */
    aiot_mqtt_setopt(handle, AIOT_MQTTOPT_EVENT_HANDLER,
                     (void*)__demo_mqtt_event_handler);
    ...
    if (res < STATE_SUCCESS) {
        ...
        DBG_I(
            "please check variables like mqtt_host, produt_key, device_name, "
            "device_secret in demo\r\n");
        return NULL;
    }
    ...
    g_mqtt_process_thread = osThreadNew(
        (osThreadFunc_t)__demo_mqtt_process_thread, handle, &mqtt_process_attr);
    ...
    g_mqtt_recv_thread = osThreadNew((osThreadFunc_t)__demo_mqtt_recv_thread,
                                     handle, &mqtt_recv_attr);
